<?php
// Connection Component Binding
Doctrine_Manager::getInstance()->bindComponent('playlist_data', 'doctrine');

/**
 * Baseplaylist_data
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $id
 * @property integer $scan_source_id
 * @property nvarchar $song_name
 * @property nvarchar $artist_name
 * @property nvarchar $album
 * @property nvarchar $country
 * @property nvarchar $genre
 * @property varchar $picture_file
 * @property varchar $youtube_link
 * @property nvarchar $record_company
 * @property nvarchar $copyright
 * @property nvarchar $release_date
 * @property boolean $processed
 * @property scan_source $scan_source
 * @property Doctrine_Collection $scan_song
 * 
 * @method integer             getId()             Returns the current record's "id" value
 * @method integer             getScanSourceId()   Returns the current record's "scan_source_id" value
 * @method nvarchar            getSongName()       Returns the current record's "song_name" value
 * @method nvarchar            getArtistName()     Returns the current record's "artist_name" value
 * @method nvarchar            getAlbum()          Returns the current record's "album" value
 * @method nvarchar            getCountry()        Returns the current record's "country" value
 * @method nvarchar            getGenre()          Returns the current record's "genre" value
 * @method varchar             getPictureFile()    Returns the current record's "picture_file" value
 * @method varchar             getYoutubeLink()    Returns the current record's "youtube_link" value
 * @method nvarchar            getRecordCompany()  Returns the current record's "record_company" value
 * @method nvarchar            getCopyright()      Returns the current record's "copyright" value
 * @method nvarchar            getReleaseDate()    Returns the current record's "release_date" value
 * @method boolean             getProcessed()      Returns the current record's "processed" value
 * @method scan_source         getScanSource()     Returns the current record's "scan_source" value
 * @method Doctrine_Collection getScanSong()       Returns the current record's "scan_song" collection
 * @method playlist_data       setId()             Sets the current record's "id" value
 * @method playlist_data       setScanSourceId()   Sets the current record's "scan_source_id" value
 * @method playlist_data       setSongName()       Sets the current record's "song_name" value
 * @method playlist_data       setArtistName()     Sets the current record's "artist_name" value
 * @method playlist_data       setAlbum()          Sets the current record's "album" value
 * @method playlist_data       setCountry()        Sets the current record's "country" value
 * @method playlist_data       setGenre()          Sets the current record's "genre" value
 * @method playlist_data       setPictureFile()    Sets the current record's "picture_file" value
 * @method playlist_data       setYoutubeLink()    Sets the current record's "youtube_link" value
 * @method playlist_data       setRecordCompany()  Sets the current record's "record_company" value
 * @method playlist_data       setCopyright()      Sets the current record's "copyright" value
 * @method playlist_data       setReleaseDate()    Sets the current record's "release_date" value
 * @method playlist_data       setProcessed()      Sets the current record's "processed" value
 * @method playlist_data       setScanSource()     Sets the current record's "scan_source" value
 * @method playlist_data       setScanSong()       Sets the current record's "scan_song" collection
 * 
 * @package    RSSMonitor
 * @subpackage model
 * @author     Anvaya Technologies
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class Baseplaylist_data extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('playlist_data');
        $this->hasColumn('id', 'integer', 5, array(
             'type' => 'integer',
             'size' => 5,
             'autoincrement' => true,
             'primary' => true,
             'length' => 5,
             ));
        $this->hasColumn('scan_source_id', 'integer', 4, array(
             'type' => 'integer',
             'size' => 4,
             'notnull' => false,
             'length' => 4,
             ));
        $this->hasColumn('song_name', 'nvarchar', 1000, array(
             'type' => 'nvarchar',
             'size' => 1000,
             'notnull' => false,
             'length' => 1000,
             ));
        $this->hasColumn('artist_name', 'nvarchar', 1000, array(
             'type' => 'nvarchar',
             'size' => 1000,
             'notnull' => false,
             'length' => 1000,
             ));
        $this->hasColumn('album', 'nvarchar', 300, array(
             'type' => 'nvarchar',
             'size' => 300,
             'notnull' => false,
             'length' => 300,
             ));
        $this->hasColumn('country', 'nvarchar', 100, array(
             'type' => 'nvarchar',
             'size' => 100,
             'notnull' => false,
             'length' => 100,
             ));
        $this->hasColumn('genre', 'nvarchar', 300, array(
             'type' => 'nvarchar',
             'size' => 300,
             'notnull' => false,
             'length' => 300,
             ));
        $this->hasColumn('picture_file', 'varchar', 300, array(
             'type' => 'varchar',
             'size' => 300,
             'notnull' => false,
             'length' => 300,
             ));
        $this->hasColumn('youtube_link', 'varchar', 1000, array(
             'type' => 'varchar',
             'size' => 1000,
             'notnull' => false,
             'length' => 1000,
             ));
        $this->hasColumn('record_company', 'nvarchar', 300, array(
             'type' => 'nvarchar',
             'size' => 300,
             'notnull' => false,
             'length' => 300,
             ));
        $this->hasColumn('copyright', 'nvarchar', 500, array(
             'type' => 'nvarchar',
             'size' => 500,
             'notnull' => false,
             'length' => 500,
             ));
        $this->hasColumn('release_date', 'nvarchar', 100, array(
             'type' => 'nvarchar',
             'size' => 100,
             'notnull' => false,
             'length' => 100,
             ));
        $this->hasColumn('processed', 'boolean', null, array(
             'type' => 'boolean',
             'notnull' => false,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('scan_source', array(
             'local' => 'scan_source_id',
             'foreign' => 'id',
             'onDelete' => 'SET NULL',
             'onUpdate' => 'CASCADE'));

        $this->hasMany('scan_song', array(
             'local' => 'id',
             'foreign' => 'playlist_data_id'));

        $timestampable0 = new Doctrine_Template_Timestampable(array(
             'created' => 
             array(
              'name' => 'created_at',
              'type' => 'timestamp',
              'options' => 
              array(
              'notnull' => false,
              ),
             ),
             'updated' => 
             array(
              'disabled' => true,
             ),
             ));
        $this->actAs($timestampable0);
    }
}